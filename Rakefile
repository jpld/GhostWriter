...
task :create_archive, [:build_path, :version] do |t, args|
  ...
  dir_name = "GhostWriter-#{args.version}"
  ...
  FileUtils.cp %w(README.markdown TODO CHANGELOG Writing\ Images.qtz), dir_name
...
task :clobber_archive do
  Dir.glob("GhostWriter-[0-9].[0-9].[0-9]*.zip").each do |f| 
    puts "removing '#{f}'"
